...
class Balloon(VectorSprite):
    """Class for awarding balloons"""
    images = []
    
    def __init__(self, layer=4, **kwargs):
        VectorSprite.__init__(self, layer=4, **kwargs)
        self.pos.y = PygView.height - 20
        self.pos.x = random.randint(10, PygView.width - 10)
        self.move = v.Vec2d(random.randint(-10,10), random.randint(-250, -150))
        self.kill_on_edge = True
    
    def create_image(self):
        self.image = random.choice(Balloon.images)
        self.rect = self.image.get_rect()
...
class PygView(object):
    ...
    def __init__(self, operator_game = "?", width=640, height=400, fps=30):
        """Initialize pygame, window, background, font,...
           default arguments """
        pygame.init()
        print(operator_game)
        self.operator = operator_game
        ...
        self.paint()
        self.points = 0 
        Balloon.images.append(pygame.image.load("ok_balloon.png").convert_alpha())
        Balloon.images.append(pygame.image.load("ok_balloon1.png").convert_alpha())
        Balloon.images.append(pygame.image.load("ok_balloon2.png").convert_alpha())
        Balloon.images.append(pygame.image.load("ok_balloon3.png").convert_alpha())
        
    def create_calculation(self):
        zahl1_addieren = random.randint(1, 50)
        zahl2_addieren = random.randint(1, 50)
        
        zahl1_subtrahieren = random.randint(1, 50)
        zahl2_subtrahieren = random.randint(1, 50)
        
        zahl1_multiplizieren = random.randint(1, 10)
        zahl2_multiplizieren = random.randint(1, 10)
        
        zahl1_dividieren = random.randint(1, 5)
        zahl2_dividieren = random.randint(1, 50)
        
        if self.operator == "+":
            text = str(zahl1_addieren) + "+" + str(zahl2_addieren)
            result = (zahl1_addieren+zahl2_addieren)
        elif self.operator == "-":
            big = max(zahl1_subtrahieren, zahl2_subtrahieren)
            small = min(zahl1_subtrahieren, zahl2_subtrahieren)
            text = str(big) + "-" + str(small)
            result = (big-small)
        elif self.operator == "*":
            text = str(zahl1_multiplizieren) + "*" + str(zahl2_multiplizieren)
            result = (zahl1_multiplizieren*zahl2_multiplizieren)
        elif self.operator == ":":
            zahl3_dividieren = zahl1_dividieren * zahl2_dividieren
            text = str(zahl3_dividieren) + "/" + str(zahl2_dividieren)
            result = (zahl1_dividieren/zahl2_dividieren)
        return text, result
    ...
    def paint(self):
        ...
        self.text, self.result = self.create_calculation()
    ...
    def run(self):
        ...
        while running:
            ...
            if user_input == str(self.result):
                ...
                self.textinput.cursor_position = 0
