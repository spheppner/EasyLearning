...
def start():
    ...
    pygame.display.set_caption("Mathematik - Multiplizieren | Drücke ESCAPE zum Verlassen!")
    ...
    def create_multiplication():
        zahl1 = random.randint(0, 15)
        zahl2 = random.randint(0, 15)
        rechnung = str(zahl1) + "*" + str(zahl2)
        text = rechnung
        return text, zahl1, zahl2

    text = create_multiplication()[0]
    ...
    while running:
        ...
        if textinput.get_text() == str((create_multiplication()[1]*create_multiplication()[2])):
            screen.fill((225, 225, 225))
            text = create_multiplication()[0]
